...
if __name__ == '__main__':
    ...
    ground_truth_rates_matrix = count_rate_scaler.fit_transform(log_ground_truth_count_rates)  # Check this! fit transform instead of transform
        ...
        with tf.Session(graph=tf.Graph()):
            ...
            for k in kernel_list:
                ...
                np.savetxt('uv_sims_stand/rss/rss_{}_iteration_{}.txt'.format(name, i), np.array(rss).reshape(-1, 1),
                           fmt='%.2f')
                file = open('uv_sims_stand/trainables/trainables_{}_and{}.txt'.format(name, i), "w")
